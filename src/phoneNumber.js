...
const toNorthAmerican = (countryCode, number) => {
  return null;
}

const toInternational = (countryCode, number) => {
  return null;
}

const parseRawNumber = (rawNumber) => {
  return null;
}
...
const phoneNumber = (rawNumber) => {
  ...
  try {
    ({countryCode, parsedNumber} = parseRawNumber(rawNumber))
    isValid = true;
  } catch (err) {
    invalidReason = err.reason;
  }
